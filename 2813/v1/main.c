...
int solve(int** items, int *pairs, int pairs_size, int itemsSize, int k)
{
    int max_elegance = calc_elegance(items, pairs, pairs_size);
    if (pairs_size == k)
    {
        return max_elegance;
    }
    else
    {
        ...
        {
            if (is_in(pairs, pairs_size, i))
            {
                continue;
            }
            pairs[pairs_size] = i;
            int cur_elegance = solve(items, pairs, pairs_size + 1, itemsSize, k);
            if (cur_elegance > max_elegance)
            {
                max_elegance = cur_elegance;
            }
        }
    }
    return max_elegance;
...
long long findMaximumElegance(int** items, int itemsSize, int k) 
{
    ...
    long long max = find_max_profit(items, itemsSize);
    indexes[0] = max;
    
    long long max_elegance = solve(items, indexes, 1, itemsSize, k);
    return max_elegance;
}
